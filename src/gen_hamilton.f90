!______SANDIP DE_____01.12.08_______________________________________________________
! Subroutine to generate the hamilton in compressed storage format
...
subroutine gen_hamilton(V)
use kind_param
use global_var
implicit none
real(R8B)::V(ndim_x,ndim_y,ndim_z)
...
do i=1,ndim_x
  do j=1,ndim_y
	...
	counter=counter+1
 !       write(*,*)"111",i,j,k
	hamilton_elements(counter)=c0+V(i,j,k)     
     enddo
  enddo
enddo
...
hamilton_elements(counter)=0  ! ARBITRARY(according to this storage rule)
do i=1,ndim_x
  do j=1,ndim_y
    do k=1,ndim_z	
      r0=((i-1)*ndim_y+j-1)*ndim_z+k
	...
	hamilton_index(index_off_first)=counter+1  ! first N places contains the location of first off diagonal
	 ...
	 if ((k+a)<=ndim_z)then
		...
		hamilton_elements(counter)=cz(a)
	...
	enddo
	 ...
	 if ((j+a)<=ndim_y)then
		...
		counter=counter+1
		index_count=index_count+1
		hamilton_elements(counter)=cy(a)
		hamilton_index(hamilton_dim+1+index_count)=r
	endif
	enddo
	 do a=1,max_coeff
	if ((i+a)<=ndim_x) then
		...
		counter=counter+1
		index_count=index_count+1
		hamilton_elements(counter)=cx(a)
		hamilton_index(hamilton_dim+1+index_count)=r
        endif
      enddo
hamilton_elements(compressed_dim)=0					! Taking
